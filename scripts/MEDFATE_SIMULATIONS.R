...
for (i in sites) {
  ...
  type<-"SINGLE" #FILTERED, #MEAN, #ALL_SINGLE #SINGLE
  sp<-"MEASURED" #(vector of Species Names for specific species) or ("MEASURED" for measured LFMC species)
  default_control<-"Sureau" #“Granier”, “Sperry”, “Cochard”, “Sureau”
  lai<-"MODIS" #MODIS (MODIS LAI DATA, CONSTATN LAY FROM THE YEAR OF MEASURED DATA) #NULL = MEDFATE
...
# 
#   sites<-CAT_FR_SITES$site_name #CHOOSE ONE
# 
# 
#   shrubData<-read.csv(paste0("data/PLOTS/",site_name,"/shrubData_ALL.csv"))
#   species<-unique(shrubData$Species) #CHOOSE ONE
#   
#   measured_year<-unique(shrubData$Date)
#   
#   CAT_LFMC<-read.csv("data/CAT_LFMC.csv")
#   
#   FR_LFMC<-read.csv("data/FR_LFMC.csv")
#   
#   measured_sp<-unique(CAT_LFMC$sp_correct_name[CAT_LFMC$LocalityName==site_name])#CHOOSE ONE
# 
# site_name<- "Port de la Selva" 
# years<-c(2021:2022) #VECTOR OF YEARS
# type<-"SINGLE" #FILTERED, #MEAN, #ALL_SINGLE #SINGLE
# sp<-"Euphorbia spp." #(vector of Species Names for specific species) or ("MEASURED" for measured LFMC species)
# default_control<-"Sureau" #“Granier”, “Sperry”, “Cochard”, “Sureau”
# lai<-"MODIS" #MODIS (MODIS LAI DATA, CONSTATN LAY FROM THE YEAR OF MEASURED DATA) #NULL = MEDFATE
# meteo<-"ERA5" #INTERPOLATORS, ERA5
# 
...
# )
# 
# SIM_data<-extract_output(SIM,LEAFPSIMAX=T,LEAFRWC=T,LFMC=T,LFMC_rodrigo=T)
# 
# name1<-paste(site_name,years[1],years[length(years)],type,sp,default_control,lai,lai_year,meteo,1, sep = "_")
# name2<-paste(site_name,years[1],years[length(years)],type,sp,default_control,lai,lai_year,meteo,2, sep = "_")
# 
# 
# write.csv(SIM_data[[1]],paste0("data/PLOTS/",site_name,"/",name1,".csv"), row.names = F)
# write.csv(SIM_data[[2]],paste0("data/PLOTS/",site_name,"/",name2,".csv"), row.names = F)
# 
# 
# #saveRDS(SIM, paste0("data/PLOTS/",site_name,"/",name,".RDS"))
# SIM<-readRDS("data/PLOTS/Badalona/Badalona_2012-2022_SINGLE_MEASURED_Sureau_MODIS_ERA5.RDS")
