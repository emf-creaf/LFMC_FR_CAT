...
evaluation_table <-function(ct_i, remove_outlier = TRUE) {
  ...
  df_full <- data.frame(site = rep(ct_filtered$site[1], length(species_measured)),
                        ...
                        lfmc = "full", 
                        n = NA, Bias = NA, Bias.rel = NA, MAE = NA, MAE.rel = NA, RMSE = NA, r2 = NA)
  df_semi <- df_full
  df_semi$lfmc <- "semi"
  for(j in 1:length(species_measured)) {
    ...
    df_full$Bias.rel[j] <- s$Bias.rel
    df_full$MAE[j] <- s$MAE
    df_full$MAE.rel[j] <- s$MAE.rel
...
for(meteo in c("ERA5", "INTER")) {
  for(lai in c("ALLOM", "MODIS")) {
    ...
    for(taw in c(30, 40, 50, 60, 80, 100, 120)) {
...
et_table_all <-et_table_all |>
  ...
  dplyr::mutate(minMAETAW = (MAE.rel == min(MAE.rel)))


et_table_best_full <- et_table_all |>
  dplyr::filter(lfmc == "full") |>
  dplyr::group_by(species, site) |>
  dplyr::filter(MAE.rel == min(MAE.rel)) 
...
# Comparison of full and semi by species
# Pooling all sources
...
species_means_optTAW <- et_table_best_semi |>
  dplyr::bind_rows(et_table_best_full)|>
  dplyr::group_by(species, lfmc) |>
  dplyr::summarise(n = dplyr::n(),
                   taw = mean(taw, na.rm = TRUE),
                   Bias = mean(Bias, na.rm = TRUE),
...
taw_means |>
  dplyr::group_by(meteo_source, lai_source, lfmc) |>
  dplyr::mutate(maxr2 = max(r2)) |>
  dplyr::filter(r2 ==maxr2)
